...
```mermaid
flowchart TD
    A[User opens TikTok] --> B[Fetch user profile\nwatch history · likes · follows]

    B --> C[Stage 1 — Candidate generation\nFAST · APPROXIMATE]
    C --> D[Pull 1000 candidate videos\nfrom billions]
    D --> E[Filter by language\nlocation · trending · follows]

    E --> F[Stage 2 — Ranking\nSLOW · ACCURATE]
    F --> G[ML model scores\nall 1000 candidates]
    G --> H[Signals used\ncompletion rate · likes\nrewatch · shares · time of day]

    H --> I[Top 10 returned to feed]
    I --> J[User watches video]
    J --> K[Completion rate recorded]
    K --> L[Profile updated]
    L -->|Next recommendation better| B
```
...
sequenceDiagram
    ...
    App->>CDN: Pre-fetch video 2 in background
    App->>CDN: Pre-fetch video 3 in background

    CDN-->>App: Video 1 ready
    Note over User,App: User watches video 1

    User->>App: Swipe up
    Note over App: Video 2 already downloaded
    App-->>User: Video 2 plays instantly

    App->>Rec: Get next video
    App->>CDN: Pre-fetch video 5 in background
```
...
flowchart LR
    ...
    subgraph Recommendation Pipeline
        J[Watch history] --> K[1000 candidates]
        K --> L[ML ranking]
        L --> M[Feed]
    end

    subgraph Analytics Pipeline
        N[Interactions] --> O[Kafka]
        O --> P[View counter]
        O --> Q[Profile updater]
        O --> R[ML trainer]
    end
